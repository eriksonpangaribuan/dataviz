...
``` r
head(iris) # mengambil 6 data awal
```
...
``` r
tail(iris) # mengambil 6 data akhir
```
...
``` r
summary(iris) # mencetak nilai statistik data yg bersifat numerik(median mean dll)
```
...
``` r
class(iris$Species) # mengklasifikasi species dari data set iris 
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # menghitung jumlah data variabel petal.witdh dari data set iris
```

    ## [1] 150

``` r
levels(iris$Species) # mengetahui level2 apa saja yang berada dalam species
```

    ## [1] "setosa"     "versicolor" "virginica"

``` r
length(levels(iris$Species)) # mencaritahu dari 150 data ada berapa variabel yang cocok
```

    ## [1] 3

``` r
unique(iris$Species) # mencaritahu data yang unik seperti levels
```
...
``` r
length(unique(iris$Species)) # dari panjang data kolom di cari tahu berapa jumlah variabel yang sama
```

    ## [1] 3

``` r
mean(iris$Sepal.Length) # mencari nilai mean
```

    ## [1] 5.843333

``` r
sd(iris$Sepal.Length) # menghitung nilai deviasi
```

    ## [1] 0.8280661

``` r
median(iris$Sepal.Width) # menghitung nilai tengah
```

    ## [1] 3

``` r
sum(iris$Petal.Length) # menghitung jumlah data
```
